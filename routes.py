...
keyboard11.row('ПРОЙТИ ОБУЧЕНИЕ', 'УЗНАТЬ ОБ ОСТАЛЬНЫХ УРОВНЯХ', 'ЗАПОЛНИТЬ РЕЗЮМЕ')
...
def confirm(message):
    bot.send_message(message.from_user.id, '''Для работы с ботом можно использовать следующие команды:
/start - начать работа
/newform - заполнение новой заявки
/forms - Ваши созданные вакансии
/myforms - взятые вами в работу заявки (для рекрутеров)
...
def update_handler(bot_instance, update):
    ...
    worker = session.query(workers).filter_by(user_id=payload_data.split()[1]).first()
    worker.closed_tasks = worker.closed_tasks + 1
    worker.closed_tasks_total = worker.closed_tasks_total + 1
    if worker.closed_tasks == 3 and worker.level == 'LIGHT':
        worker.level = 'MEDIUM'
        worker.closed_tasks = 0
    elif worker.closed_tasks == 7 and worker.level == 'MEDIUM':
        worker.level = 'HARD'
        worker.closed_tasks = 0
    elif worker.closed_tasks == 20 and worker.level == 'HARD':
        worker.level = 'PRO'
    session.commit()
...
''',
                         reply_markup=keyboard11)
    elif message.text == 'ЗАПОЛНИТЬ РЕЗЮМЕ':
        if len(session.query(workers).filter_by(user_id=message.from_user.id).all()) == 0:
            session.add(workers(user_id=message.from_user.id))
            worker = session.query(workers).filter_by(user_id=message.from_user.id).first()
            worker.level = 'moderate'
            session.commit()
            session.close()
        bot.send_message(message.from_user.id,
...
''',
    ...
    #получение заявок
        ...
        for s_form in session.query(form).filter_by(pay_level=worker_level, active = True, archive = False).all():
            ...
            Сумма вознаграждения: {6},
            Взято в работу рекрутерами: {8}.
            Ваш уровень: закрытых заявок {9} ({10} заявки до перехода на уровень {12})
                    '''.format(
                str(s_form.id),
                s_form.vacancy,
                s_form.duties,
...
def get_callback(call):
    ...
    elif call.data.split()[0] == 'mod':
        level = call.data.split()[1]
        work_id = call.data.split()[2]
        worker = session.query(workers).filter_by(user_id=work_id).first()
        worker.level = level
        worker.test_stage = 4
        worker.done = True
        session.commit()
